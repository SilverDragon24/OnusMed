...
Public Class GSTR3BReport
    Private Sub btnCustomReport_Click(sender As Object, e As EventArgs) Handles btnCustomReport.Click
        ...
        Dim folderpath As String = String.Concat(docpath, "\Onus_GSTR3B")
        If (Not Directory.Exists(folderpath)) Then
            Directory.CreateDirectory(folderpath)
        End If
        Dim now As DateTime = DateTime.Now
        Dim filepath As String = String.Concat(folderpath, "\Onus_GSTR-3B_", now.ToString("MM.dd.yyyy_HH.mm.ss"), ".html")
        ...
        file.WriteLine("<html>")
        file.WriteLine("<title>GSTR-3B Report</title>")
        file.WriteLine("<head>")
        ...
        file.WriteLine("</head>")
        file.WriteLine("<body>")
        file.WriteLine("<center><h1>GSTR-3B Report</h1></center>")
        ...
        selectData(String.Concat(New String() {"select invoice_no from purchase where (idate between '", dfrom, "' and '", dto, "');"}), invoices)
        file.WriteLine("<tr><th>Invoice No.</th><th>Date</th><th>Supplier</th><th>Taxable Amount</th><th>CGST</th><th>SGST</th><th>IGST</th></tr>")
        Dim ino As String = ""
        Dim idate As String = ""
        Dim name As String = ""
        ...
        Try
            If (invoices.Tables(0).Rows.Count > 0) Then
                ...
                Do
                    ...
                    selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase where invoice_no='", invoices.Tables(0).Rows(i)(0).ToString(), "';"), info)
                    selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase_r where invoice_no='", invoices.Tables(0).Rows(i)(0).ToString(), "';"), returns)
    ...
    Private Sub btnQReport_Click(sender As Object, e As EventArgs) Handles btnQReport.Click
        ...
        If (radioQ1.Checked) Then
            ...
            Dim str1 As String = String.Concat(str, "\Onus_GSTR3B")
            If (Not Directory.Exists(str1)) Then
                Directory.CreateDirectory(str1)
            End If
            Dim strArrays() As String = {str1, "\Onus_GSTR3B_Q1_", num2.ToString(), "-", Nothing, Nothing, Nothing, Nothing}
            num = num2 + 1
            ...
            Dim strArrays1() As String = {"<title>GSTR-3B Report for Quarter 1 of ", num2.ToString(), "-", Nothing, Nothing}
            num = num2 + 1
            ...
            Dim strArrays2() As String = {"<center><h1>GSTR-3B Report for Quarter 1 of ", num2.ToString(), "-", Nothing, Nothing}
            num = num2 + 1
            ...
            today = dateTo.Value
            today.ToString("yyyy-MM-dd")
            selectData(String.Concat(New String() {"select invoice_no from purchase where (idate between '", num2.ToString(), "-04-01' and '", num2.ToString(), "-06-31');"}), dataSet)
            ...
            Try
                If (dataSet.Tables(0).Rows.Count > 0) Then
                    ...
                    Do
                        ...
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase where invoice_no='", dataSet.Tables(0).Rows(num15)(0).ToString(), "';"), dataSet1)
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase_r where invoice_no='", dataSet.Tables(0).Rows(num15)(0).ToString(), "';"), dataSet2)
        ...
        ElseIf (radioQ2.Checked) Then
            ...
            Dim str7 As String = String.Concat(str6, "\Onus_GSTR3B")
            If (Not Directory.Exists(str7)) Then
                Directory.CreateDirectory(str7)
            End If
            Dim strArrays3() As String = {str7, "\Onus_GSTR3B_Q2_", num17.ToString(), "-", Nothing, Nothing, Nothing, Nothing}
            num = num17 + 1
            ...
            Dim strArrays4() As String = {"<title>GSTR-3B Report for Quarter 2 of ", num17.ToString(), "-", Nothing, Nothing}
            num = num17 + 1
            ...
            Dim strArrays5() As String = {"<center><h1>GSTR-3B Report for Quarter 2 of ", num17.ToString(), "-", Nothing, Nothing}
            num = num17 + 1
            ...
            today = dateTo.Value
            today.ToString("yyyy-MM-dd")
            selectData(String.Concat(New String() {"select invoice_no from purchase where (idate between '", num17.ToString(), "-07-01' and '", num17.ToString(), "-09-31');"}), dataSet3)
            ...
            Try
                If (dataSet3.Tables(0).Rows.Count > 0) Then
                    ...
                    Do
                        ...
                        Dim dataSet5 As System.Data.DataSet = New System.Data.DataSet()
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase where invoice_no='", dataSet3.Tables(0).Rows(num30)(0).ToString(), "';"), dataSet4)
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase_r where invoice_no='", dataSet3.Tables(0).Rows(num30)(0).ToString(), "';"), dataSet5)
        ...
        ElseIf (radioQ3.Checked) Then
            ...
            Dim str13 As String = String.Concat(str12, "\Onus_GSTR3B")
            If (Not Directory.Exists(str13)) Then
                Directory.CreateDirectory(str13)
            End If
            Dim strArrays6() As String = {str13, "\Onus_GSTR3B_Q3_", num32.ToString(), "-", Nothing, Nothing, Nothing, Nothing}
            num = num32 + 1
            ...
            Dim strArrays7() As String = {"<title>GSTR-3B Report for Quarter 3 of ", num32.ToString(), "-", Nothing, Nothing}
            num = num32 + 1
            ...
            Dim strArrays8() As String = {"<center><h1>GSTR-3B Report for Quarter 3 of ", num32.ToString(), "-", Nothing, Nothing}
            num = num32 + 1
            ...
            Dim dataSet6 As System.Data.DataSet = New System.Data.DataSet()
            today = dateFrom.Value
            today.ToString("yyyy-MM-dd")
            today = dateTo.Value
            today.ToString("yyyy-MM-dd")
            selectData(String.Concat(New String() {"select invoice_no from purchase where (idate between '", num32.ToString(), "-10-01' and '", num32.ToString(), "-12-31');"}), dataSet6)
            ...
            Try
                If (dataSet6.Tables(0).Rows.Count > 0) Then
                    ...
                    Do
                        Dim dataSet7 As System.Data.DataSet = New System.Data.DataSet()
                        Dim dataSet8 As System.Data.DataSet = New System.Data.DataSet()
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase where invoice_no='", dataSet6.Tables(0).Rows(num45)(0).ToString(), "';"), dataSet7)
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase_r where invoice_no='", dataSet6.Tables(0).Rows(num45)(0).ToString(), "';"), dataSet8)
        ...
        ElseIf (radioQ4.Checked) Then
            ...
            If (Not Directory.Exists(folderpath)) Then
                Directory.CreateDirectory(folderpath)
            End If
            Dim strArrays9() As String = {folderpath, "\Onus_GSTR3B_Q4_", year.ToString(), "-", Nothing, Nothing, Nothing, Nothing}
            num = year + 1
            ...
            file.WriteLine("<html>")
            Dim strArrays10() As String = {"<title>GSTR-3B Report for Quarter 4 of ", year.ToString(), "-", Nothing, Nothing}
            num = year + 1
            ...
            file.WriteLine("</head>")
            file.WriteLine("<body>")
            Dim strArrays11() As String = {"<center><h1>GSTR-3B Report for Quarter 4 of ", year.ToString(), "-", Nothing, Nothing}
            num = year + 1
            strArrays11(3) = num.ToString()
            strArrays11(4) = "</h1></center>"
            file.WriteLine(String.Concat(strArrays11))
            file.WriteLine("<table border=1 width=100%>")
            Dim invoices As System.Data.DataSet = New System.Data.DataSet()
            today = dateFrom.Value
            today.ToString("yyyy-MM-dd")
            today = dateTo.Value
            today.ToString("yyyy-MM-dd")
            Dim strArrays12() As String = {"select invoice_no from purchase where (idate between '", Nothing, Nothing, Nothing, Nothing}
            num = year + 1
            ...
            Try
                If (invoices.Tables(0).Rows.Count > 0) Then
                    ...
                    Dim i As Integer = 0
                    Do
                        Dim info As System.Data.DataSet = New System.Data.DataSet()
                        Dim returns As System.Data.DataSet = New System.Data.DataSet()
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase where invoice_no='", invoices.Tables(0).Rows(i)(0).ToString(), "';"), info)
                        selectData(String.Concat("select invoice_no,date_format(idate,'%d-%b-%Y'),(select name from supplier where id=suppl),(piece_q*pr_piece),(cgst/100)*(piece_q*pr_piece),(sgst/100)*(piece_q*pr_piece),(igst/100)*(piece_q*pr_piece) from purchase_r where invoice_no='", invoices.Tables(0).Rows(i)(0).ToString(), "';"), returns)
                        ino = info.Tables(0).Rows(0)(0).ToString()
                        idate = info.Tables(0).Rows(0)(1).ToString()
    ...
    Private Sub GSTR3BReport_Load(sender As Object, e As EventArgs) Handles Me.Load
        Timer1.Interval = 50
        Timer1.Start()
    End Sub
