...
Public Class PurchaseReturnReport
    Private Sub btnRequest_Click(sender As Object, e As EventArgs) Handles btnRequest.Click
        ...
        Dim folderpath As String = String.Concat(docpath, "\PurchaseReturnReports")
        If (Not Directory.Exists(folderpath)) Then
            Directory.CreateDirectory(folderpath)
        End If
        Dim str() As String = {folderpath, "\PurchaseReturnReport_", Nothing, Nothing, Nothing, Nothing, Nothing, Nothing}
        ...
        ElseIf (radioSupplier.Checked) Then
            ...
            If (suppliers.Tables(0).Rows.Count > 0) Then
                ...
                For i As Integer = 0 To count2 Step 1
                    ...
                    Dim ppack As Double = 0
                    Dim pstrip As Double = 0
                    Dim ppiece As Double = 0
                    Dim fpack As Double = 0
                    Dim fstrip As Double = 0
                    Dim fpiece As Double = 0
                    Dim amt As Double = 0
                    If (ritems.Tables(0).Rows.Count > 0) Then
    ...
    Private Sub btnToday_Click(sender As Object, e As EventArgs) Handles btnToday.Click
        Dim num As Double
        Dim docpath As String = Environment.GetFolderPath(Environment.SpecialFolder.Personal).ToString()
        Dim folderpath As String = String.Concat(docpath, "\PrchaseReturnReports")
        If (Not Directory.Exists(folderpath)) Then
            Directory.CreateDirectory(folderpath)
        End If
        Dim now As DateTime = DateTime.Now
        Dim filepath As String = String.Concat(folderpath, "\DailyPurchaseReturnReport_", now.ToString("MM.dd.yyyy_HH.mm.ss"), ".html")
        Using fileStream As System.IO.FileStream = System.IO.File.Create(filepath)
        End Using
        Dim file As StreamWriter = New StreamWriter(filepath)
        file.WriteLine("<html>")
        now = DateTime.Today
        file.WriteLine(String.Concat("<title>Daily Purchase Return Report for ", now.ToString("MM-dd-yyyy"), "</title>"))
        file.WriteLine("<head>")
        file.WriteLine("<style media=""screen"" type=""text/css"">")
        file.WriteLine("@media print {")
        file.WriteLine("thead {display: table-header-group;}")
